...
unsigned long timeSensor = 0; // Timestamp that updates each loop() iteration
bool toggleSensor = true; // Flag to toggle between the two sensors
...
void loop()
{
  ...
  // When it is time to read and publish one of the two sensor output values
  if (millis() - timeSensor >= intervalPublish)
  {
    // And depending on the sensor toggle flag status
    if (toggleSensor)
    {
      readSPS30(); // First, call the function that reads a value from the first sensor
      ...
      toggleSensor = !toggleSensor; // Invert the toggle flag so that next time the other sensor will be read and published

      timeSensor = millis(); // Finally, update the timestamp for the next loop() iteration
    }
    else
    {
      readSCD30();
